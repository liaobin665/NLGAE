...
cora_pd = pd.read_csv("../../mydata/palanetoidEmbeding/cora_deepwalk_Embeding_256wei.csv")
coraX = pd.read_csv('../getDataX/cora_X.csv')
cora_pd = pd.merge(coraX,cora_pd,left_on='node_id',right_index=True)

citeseer_pd = pd.read_csv("../../mydata/palanetoidEmbeding/citeseer_deepwalk_Embeding_256wei.csv")
citeseerX=pd.read_csv('../getDataX/citeseer_X.csv')
citeseer_pd = pd.merge(citeseerX,citeseer_pd,left_on='node_id',right_index=True)

pubmed_pd = pd.read_csv("../../mydata/palanetoidEmbeding/pubmed_deepwalk_Embeding_256wei.csv")
pubmedX=pd.read_csv('../getDataX/pubmed_X.csv')
pubmed_pd = pd.merge(pubmedX,pubmed_pd,left_on='node_id',right_index=True)


amazon_cs_pd = pd.read_csv("../../mydata/NPZEmbeding/amazon_cs_deepwalk_Embeding_256wei.csv")
amazon_csX=pd.read_csv('../getDataX/amazon_cs_X.csv')
amazon_cs_pd = pd.merge(amazon_csX,amazon_cs_pd,left_on='node_id',right_index=True)

dblp_pd = pd.read_csv("../../mydata/NPZEmbeding/dblp_deepwalk_Embeding_256wei.csv")
dblpX=pd.read_csv('../getDataX/dblp_X.csv')
dblp_pd = pd.merge(dblpX,dblp_pd,left_on='node_id',right_index=True)

twitch_pd = pd.read_csv("../../mydata/karateclubEmbeding/twitch_deepwalk_Embeding_256wei.csv")
twitchX=pd.read_csv('../getDataX/twitch_X.csv')
twitch_pd = pd.merge(twitchX,twitch_pd,left_on='node_id',right_index=True)


uai_pd = pd.read_csv("../../mydata/NPZEmbeding/uai_deepwalk_Embeding_256wei.csv")
uaiX=pd.read_csv('../getDataX/uai_X.csv')
uai_pd = pd.merge(uaiX,uai_pd,left_on='node_id',right_index=True)
...
performaceFileName = "deepwalk+X+lr_result.csv"
